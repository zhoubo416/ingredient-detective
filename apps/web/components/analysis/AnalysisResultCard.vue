  ...
  <div class="space-y-3">
    ...
    <template v-else>
      ...
      <div class="rounded-2xl bg-gradient-to-br from-[#f3fbf4] to-[#e7f4e9] border border-[#d9e8db] p-4">
        ...
        <!-- 个性化提醒 -->
        <div v-if="result?.recommendations" class="mt-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3">
          <div class="flex items-center gap-1.5 mb-1.5">
            <UIcon name="i-lucide-user" class="size-3.5 text-amber-700" />
            <span class="text-xs font-semibold text-amber-800">对你的提醒</span>
          </div>
          <p class="text-sm leading-relaxed text-amber-900">{{ result.recommendations }}</p>
        </div>
      </div>
